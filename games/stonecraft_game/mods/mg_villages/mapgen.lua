...
mg_villages.lower_or_raise_terrain_at_point = function( x, z, target_height, minp, maxp, vm, data, param2_data, a, cid, vh, treepos, has_artificial_snow, blend, force_ground, force_underground )
	...
	if( minp.y > vh ) then
		...
		for y=minp.y, minp.y+16 do
			if(vm:get_data_from_heap(data, a:index( x, y, z )) == cid.c_air ) then
	...
	if( not( surface_node ) and old_height == maxp.y ) then
		if(     vm:get_data_from_heap(data, a:index( x, minp.y, z))==cid.c_air) then
			old_height = vh - 2;	
	...
	if( target_height >= minp.y and target_height < maxp.y ) then
		...
		elseif( tree  and not( mg_villages.ethereal_trees ) and treepos) then
			vm:set_data_from_heap(data,        a:index( x, target_height+1, z), cid.c_sapling)
		...
		elseif( jtree and not( mg_villages.ethereal_trees ) and treepos) then
			vm:set_data_from_heap(data,        a:index( x, target_height+1, z), cid.c_jsapling)
		...
		elseif( ptree and not( mg_villages.ethereal_trees ) and treepos) then
			vm:set_data_from_heap(data,        a:index( x, target_height+1, z), cid.c_psapling)
		...
		elseif( atree and not( mg_villages.ethereal_trees ) and treepos) then
			vm:set_data_from_heap(data,        a:index( x, target_height+1, z), cid.c_asapling)
		...
		vm:set_data_from_heap(data,                a:index( x, target_height,   z), surface_node);
		if( target_height-1 >= minp.y ) then
			vm:set_data_from_heap(data,        a:index( x, target_height-1, z), below_1);
		end
	end
	...
	while( y > minp.y and y > target_height-40 and y <=maxp.y) do
		local old_node = vm:get_data_from_heap(data, a:index( x, y, z));
		...
		if( old_node == cid.c_air or old_node == cid.c_water ) then
			...
			if( y == coal_height1 or y == coal_height2 ) then
				vm:set_data_from_heap(data, a:index( x, y, z ), cid.c_stone_with_coal);
			else
				vm:set_data_from_heap(data, a:index( x, y, z), below_2);
			end
			y = y-1;
		else
			y = minp.y - 1;
...
mg_villages.repair_outer_shell = function( villages, minp, maxp, vm, data, param2_data, a, village_area, cid, edge_min, edge_max )
	...
	if(    vm:get_data_from_heap(data, a:index(minp.x,minp.y,minp.z)) == cid.c_ignore

	   and vm:get_data_from_heap(data, a:index(maxp.x,minp.y,minp.z)) == cid.c_ignore
	   and vm:get_data_from_heap(data, a:index(minp.x,maxp.y,minp.z)) == cid.c_ignore
	   and vm:get_data_from_heap(data, a:index(minp.x,minp.y,maxp.z)) == cid.c_ignore

	   and vm:get_data_from_heap(data, a:index(maxp.x,maxp.y,maxp.z)) == cid.c_ignore

	   and vm:get_data_from_heap(data, a:index(maxp.x,maxp.y,minp.z)) == cid.c_ignore
	   and vm:get_data_from_heap(data, a:index(maxp.x,minp.y,maxp.z)) == cid.c_ignore
	   and vm:get_data_from_heap(data, a:index(minp.x,maxp.y,maxp.z)) == cid.c_ignore ) then
	...
	for x = minp.x, maxp.x do
		...
		if( village_area[ x ][ z ][ 2 ] > 0 ) then
			...
			local node    = vm:get_data_from_heap(data, a:index(x,village.vh,z));
			...
			if( village and village.vh and (node==cid.c_air or node==cid.c_water)) then 
				...
				while( y > minp.y ) do
					...
					if(     ci == cid.c_desert_stone or ci == cid.c_desert_sand ) then
						vm:set_data_from_heap(data, a:index(x, village.vh, z), cid.c_desert_sand);
						y = minp.y-1;
					elseif( ci == cid.c_sand ) then
						vm:set_data_from_heap(data, a:index(x, village.vh, z), cid.c_sand);
						y = minp.y-1;
					elseif( ci == cid.c_silver_sand or ci == cid.c_silver_sandstone) then
						vm:set_data_from_heap(data, a:index(x, village.vh, z), cid.c_silver_sand);
						y = minp.y-1;
					...
					elseif( ci ~= cid.c_air and ci ~= cid.c_ignore and ci ~= cid.c_water and mg_villages.check_if_ground( ci ) == true) then
						vm:set_data_from_heap(data, a:index(x, village.vh, z), cid.c_dirt_with_grass);
						y = minp.y-1;
...
mg_villages.village_area_get_height = function( village_area, villages, minp, maxp, vm, data, param2_data, a, cid )
...
mg_villages.grow_a_tree = function( pos, plant_id, minp, maxp, data, a, cid, pr, snow )
	...
	elseif( plant_id == cid.c_jsapling and minetest.registered_nodes[ 'default:jungletree']) then
		mg_villages.grow_jungletree( vm, data, a, pos, math.random(1,100000), snow)
		return true;
	-- a pine tree
	elseif( plant_id == cid.c_psapling and minetest.registered_nodes[ 'default:pine_tree']) then
		mg_villages.grow_pinetree(   vm, data, a, pos, snow);
		return true;
	...
	elseif( plant_id == cid.c_savannasapling and mg_villages.add_savannatree) then
		mg_villages.add_savannatree(         vm, data, a, pos.x, pos.y, pos.z, minp, maxp, pr) -- TODO: snow
		return true;
	-- a pine tree from the mg mod
	elseif( plant_id == cid.c_pinesapling    and mg_villages.add_pinetree   ) then
		mg_villages.add_pinetree(            vm, data, a, pos.x, pos.y, pos.z, minp, maxp, pr) -- TODO: snow
		return true;
	end
	return false;
end
...
mg_villages.village_area_fill_with_plants = function( village_area, villages, minp, maxp, vm, data, param2_data, a, cid )
		...
		for z = minp.z, maxp.z do
			...
			if( village_area[ x ][ z ][ 2 ]==1 
			 or village_area[ x ][ z ][ 2 ]==6) then
				...
				local h = village.vh;
				local g = vm:get_data_from_heap(data, a:index( x, h, z ));
				...
				local plant_id = vm:get_data_from_heap(data, a:index( x, h+1, z));
				local on_soil  = false;
				...
				elseif( on_soil and (g==cid.c_dirt_with_grass or g==cid.c_soil_wet or g==cid.c_dirt_with_snow)) then	
					...
					if( has_snow_cover and mg_villages.use_soil_snow) then
						vm:set_data_from_heap(data, a:index( x,  h+1, z), cid.c_msnow_soil);
						has_snow_cover = nil;
					else
						vm:set_data_from_heap(data, a:index( x,  h+1, z), plant_id);
					end
				...
				elseif( on_soil ) then
					if( math.random(1,5)==1 ) then
						vm:set_data_from_heap(data, a:index( pos.x,  pos.y, pos.z), cid.c_shrub);
					end

				elseif( plant_id ) then -- place the sapling or plant (moretrees uses spawn_tree)
					vm:set_data_from_heap(data, a:index( pos.x,  pos.y, pos.z), plant_id);
				end

				-- put a snow cover on plants where needed
				if( has_snow_cover and cid.c_msnow_1 ~= cid.c_ignore) then
					vm:set_data_from_heap(data, a:index( x,  h+2, z), cid.c_msnow_1);
				end
...
mg_villages.place_villages_via_voxelmanip = function( villages, minp, maxp, vm, data, param2_data, a, top, seed )
	...
	mg_villages.village_area_get_height( village_area, villages, tmin, tmax, vm, data, param2_data, a, cid );
...
--	mg_villages.repair_outer_shell(   villages, tmin, tmax, vm, data, param2_data, a, village_area, cid );
	...
	for _, village in ipairs(villages) do
		...
		village.to_add_data = handle_schematics.place_buildings( village, tmin, tmax, vm, data, param2_data, a, cid, village_id);

		handle_schematics.place_dirt_roads(                village, tmin, tmax, vm, data, param2_data, a, c_feldweg);

		-- grow trees which are part of buildings into saplings
		for _,v in ipairs( village.to_add_data.extra_calls.trees ) do
			mg_villages.grow_a_tree( v, v.typ, minp, maxp, vm, data, a, cid, nil, v.snow ); -- TODO: supply pseudorandom value?
		end
	end
