...
function subterrane:register_cave_layer(cave_layer_def)
	...
	minetest.register_on_generated(function(minp, maxp, seed)
		...
		data_param2 = vm:load_param2_data_into_heap()
			...
			for y = y_min, y_max do -- for each x row progressing upwards
				...
				for x = x_min, x_max do -- for each node do
					...
					if cave_value > tcave then --if node falls within cave threshold
						if cave_value > tcave then
							if column_value > 0 and cave_value - column_value * column_weight < tcave then
								vm:set_data_from_heap(data, vi, column_node) -- add a column
							else
								vm:set_data_from_heap(data, vi, fill_node) --hollow it out to make the cave
							end
						end
					elseif biome and biome._subterrane_cave_fill_node and vm:get_data_from_heap(data, vi) == c_air then
						vm:set_data_from_heap(data, vi, biome._subterrane_cave_fill_node)
					end
					...
					if biome and biome._subterrane_mitigate_lava and cave_value > tcave - 0.1 then -- Eliminate nearby lava to keep it from spilling in
						if vm:get_data_from_heap(data, vi) == c_lava then
							vm:set_data_from_heap(data, vi, c_obsidian)
						end
					end
					--increment indices
					index_3d = index_3d + 1
					...
					if biome then
						...
						if math.floor(cave_value*30) == math.floor(tcave*30) then
							...
							if biome._subterrane_ceiling_decor
								and vm:get_data_from_heap(data, ai) ~= fill_node
								and vm:get_data_from_heap(data, vi) == fill_node
								and y < y_max
								then --ceiling
								biome._subterrane_ceiling_decor(vm, area, data, ai, vi, bi, data_param2)
							end
							--ground
							if biome._subterrane_floor_decor
								and vm:get_data_from_heap(data, bi) ~= fill_node
								and vm:get_data_from_heap(data, vi) == fill_node
								and y > y_min
								then --ground
								biome._subterrane_floor_decor(vm, area, data, ai, vi, bi, data_param2)
							end
						...
						elseif cave_value <= tcave then --if node falls outside cave threshold
							...
							if biome._subterrane_cave_fill_node then
								...
								if vm:get_data_from_heap(data, vi) == c_air then
									vm:set_data_from_heap(data, vi, cave_fill_node)
								end
							end
							...
							if biome._subterrane_cave_ceiling_decor
								and vm:get_data_from_heap(data, ai) ~= cave_fill_node
								and vm:get_data_from_heap(data, vi) == cave_fill_node
								and y < y_max
								then --ceiling
								biome._subterrane_cave_ceiling_decor(vm, area, data, ai, vi, bi, data_param2)
							end
							if biome._subterrane_cave_floor_decor
								and vm:get_data_from_heap(data, bi) ~= cave_fill_node
								and vm:get_data_from_heap(data, vi) == cave_fill_node
								and y > y_min
								then --ground
								biome._subterrane_cave_floor_decor(vm, area, data, ai, vi, bi, data_param2)
							end
						end	
					end
					index_3d = index_3d + 1
		...
		vm:save_param2_data_from_heap(data_param2)
		--calc lighting
