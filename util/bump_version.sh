...
perform_release() {
	...
	sed -i '/\<release/s/\(version\)="[^"]*"/\1="'"$RELEASE_VERSION"'"/' misc/net.stonecraft.stonecraft.appdata.xml
	sed -i 's/\(<release date\)="[^"]*"/\1="'"$RELEASE_DATE"'"/' misc/net.stonecraft.stonecraft.appdata.xml

	git add -f CMakeLists.txt build/android/build.gradle misc/net.stonecraft.stonecraft.appdata.xml
